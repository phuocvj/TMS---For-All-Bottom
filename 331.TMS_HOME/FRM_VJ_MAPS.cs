...
namespace FORM
{
    public partial class FRM_VJ_MAPS : Form
    {
        ...
        //info car new
        int X_Car1, Y_Car1, X_Car2, Y_Car2, X_Car3, Y_Car3,X_Car4,Y_Car4;
        ...
        Point ORI_CAR4 = new Point(1518, 134);  // car tp
            ...
            if (btnCar.InvokeRequired == true)
                btnCar.Invoke((MethodInvoker)delegate
                {
                    try
                    {
                        ...
                        //Xuất phát từ TÂN PHÚ -> Long Thành
                        if (!string.IsNullOrEmpty(btnCar4.Text))
                        {
                            if (!string.IsNullOrEmpty(TP_LT_Departtime))
                            {
                                DateTime startTime = new DateTime(DateTime.Now.Year, DateTime.Now.Month, DateTime.Now.Day, Convert.ToInt32(TP_LT_Departtime.Substring(0, 2)), Convert.ToInt32(TP_LT_Departtime.Substring(3, 2)), 00);
                                DateTime endTime = new DateTime(DateTime.Now.Year, DateTime.Now.Month, DateTime.Now.Day,
                                Convert.ToInt32(DateTime.Now.ToString("HH")), Convert.ToInt32(DateTime.Now.ToString("mm")), 00);
                                TimeSpan span = endTime.Subtract(startTime);
                                minutes = Convert.ToInt32(span.TotalMinutes);

                                if (minutes >= 180 || TP_LT_Arr.Equals("1"))
                                {
                                    X_Car4 = 975;
                                    Y_Car4 = 746;
                                ...
                                else
                                {
                                    Y_Car4 = minutes * 3;
                                    X_Car4 = minutes * 3;
                                    btnCar4.Location = new Point(ORI_CAR4.X - X_Car4 + 80, ORI_CAR4.Y + Y_Car4);
                                }
                            }
                            else
                            {
        ...
        private DataTable Select_TP_LT_Train_Time(string Qtype, string Factory)
        {
            try
            {
                COM.OraDB MyOraDB = new COM.OraDB();
                System.Data.DataSet ds_ret;
                string process_name = "MES.PKG_TMS_HOME.TMS_VJ3_VJ23_DEPART_TIME";
                MyOraDB.ReDim_Parameter(3);
                MyOraDB.Process_Name = process_name;
                MyOraDB.Parameter_Name[0] = "ARG_QTYPE";
                MyOraDB.Parameter_Name[1] = "ARG_FAC";
        ...
        private DataTable Select_TP_LT_qty_Trip()
        {
            try
            {
                COM.OraDB MyOraDB = new COM.OraDB();
                ...
                MyOraDB.Parameter_Values[0] = DateTime.Now.ToString("yyyyMMdd");
                MyOraDB.Parameter_Values[1] = "VJ3_LT";
                MyOraDB.Parameter_Values[2] = "";
        ...
        public void GetDepartTime()
        {
            ...
            dtTPLT = Select_TP_LT_Train_Time("Q", "VJ3_LT");
            dtQuaTPLT = Select_TP_LT_qty_Trip();
            ...
            //TAN PHU -> LONG THANH
            if (dtTPLT.Rows.Count > 0 && dtTPLT != null)
            {
                TP_LT_Departtime = dtTPLT.Rows[0]["DP_TIME"].ToString();
                TP_LT_Arr = dtTPLT.Rows[0]["ARR_YN"].ToString();
            }
            if (dtQuaTPLT.Rows.Count > 0 && dtQuaTPLT != null)
            {
                if (dtQuaTPLT.Rows[0]["QTY"].Equals("0") || dtQuaTPLT.Rows[0]["CUR_TRIP"].Equals("0"))
                    btnCar4.Text = "";
                else
                {
                    string Qty = string.Concat("Trip: 1", "\n", string.Format("{0:n0}", dtQuaTPLT.Rows[0]["QTY"]), " Prs");
                    btnCar4.Text = Qty + "\nVJ3->VJ2";
                }
            }
            else
