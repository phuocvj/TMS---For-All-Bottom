...
namespace FORM
{
    public partial class FRM_BT_SET_BALANCE : Form
    {
        ...
        private void BindingData()
        {
            ...
            gvwBase_set.Columns["SET_RATIO"].Visible = false;
            ...
            {
                if (i <3)
                    gvwBase_set.Columns[i].OptionsColumn.AllowMerge = DevExpress.Utils.DefaultBoolean.True;
                else
                    gvwBase_set.Columns[i].OptionsColumn.AllowMerge = DevExpress.Utils.DefaultBoolean.False;
                
                gvwBase_set.Columns[i].AppearanceCell.TextOptions.HAlignment = HorzAlignment.Center;
            }
        ...
        private void gvwBase_set_RowCellStyle(object sender, DevExpress.XtraGrid.Views.Grid.RowCellStyleEventArgs e)
        {
            if (e.Column.FieldName == "SET")
            {
                if (!string.IsNullOrEmpty(gvwBase_set.GetRowCellValue(e.RowHandle, "SET_RATIO").ToString()))
                {
                    double Ratio = Convert.ToDouble(gvwBase_set.GetRowCellValue(e.RowHandle, "SET_RATIO"));
                    if (Ratio >= 98)
                    {
