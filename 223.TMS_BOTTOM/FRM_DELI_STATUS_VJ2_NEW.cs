...
namespace FORM
{
    public partial class FRM_DELI_STATUS_VJ2_NEW : Form
    {
        public FRM_DELI_STATUS_VJ2_NEW()
        {
            InitializeComponent();
        }
        string OP_CD = "OSP";
        ...
        private void FRM_DELI_STATUS_V3_VisibleChanged(object sender, EventArgs e)
        {
            if (this.Visible)
            {
                ...
                lblTitle.Text = string.Concat(string.IsNullOrEmpty(ComVar.Var._Frm_Back) ? ComVar.Var._strValue3 : ComVar.Var._strValue4, " OUTSOLE - TMS DETAIL");
            ...
            foreach (DevExpress.XtraEditors.SimpleButton button in groupBoxEx1.Controls)
            {
                ...
                if (button.Tag.ToString().Equals(((DevExpress.XtraEditors.SimpleButton)sender).Tag.ToString()))
                {
                    try
                    {
                        ...
                        lblTitle.Text = string.Concat(string.IsNullOrEmpty(ComVar.Var._Frm_Back) ? ComVar.Var._strValue2 : ComVar.Var._strValue3, " ", CMP_NM, " - TMS DETAIL");
